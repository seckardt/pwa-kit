...
import {createShopperBasketsClient} from '@/app/utils/api/commerce-client'

export async function loader({request}: LoaderFunctionArgs): Promise<ShopperBasketsTypes.Basket> {
    ...
    try {
        ...
        if (typeof error === 'object' && error !== null && Reflect.has(error, 'response')) {
            const response = (Reflect.get(error, 'response') as Response).clone()
            ...
            if (
                ...
                const id = (
                    Array.isArray(basketIds) ? basketIds : [String(basketIds as string)]
                ).at(0) as string
                const basket = await client.getBasket({
                    parameters: {
                        basketId: id
